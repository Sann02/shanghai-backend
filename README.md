...
|--------|----------|-----------|-------------|
| POST   | `/auth/login` | Login, mendapatkan access & refresh token | Publik |
| POST   | `/auth/refresh` | Memperbarui access token dengan refresh token | Refresh token |
| POST   | `/users/register` | Registrasi user baru | Publik |
| GET    | `/users/<user_id>` | Ambil detail user berdasarkan ID | Publik |
| GET    | `/store/products` | Ambil daftar produk (pagination & filter) | Publik |
| POST   | `/store/products` | Buat produk baru | seller |
| GET    | `/store/products/<product_id>` | Ambil detail produk berdasarkan ID | Publik |
| PUT    | `/store/products/<product_id>` | Update produk | seller |
| DELETE | `/store/products/<product_id>` | Hapus produk | admin |
| GET    | `/store/categories/<category_id>` | Ambil kategori beserta produknya | Terautentikasi (JWT) |
| GET    | `/orders` | Ambil semua order | user |
| GET    | `/orders/<order_id>` | Ambil detail order berdasarkan ID | user |
| POST   | `/orders` | Buat order baru | user |

> Untuk endpoint yang butuh auth, sertakan header `Authorization: Bearer <access_token>`.
